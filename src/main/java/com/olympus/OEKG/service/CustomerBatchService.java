...
public class CustomerBatchService {
	...
	final Logger LOGGER = LoggerFactory.getLogger(CustomerBatchService.class);

	private static utility objUtil = new utility();

	public static Integer PARENT_ID = 0;
	public static Integer CHILD_BATCH_ID = 0;
	public static List<Integer> BATCH_LIST = new ArrayList<Integer>();

	private SchedulerCust scheduler = SchedulerCust.getSchedulerObj();
	public static List<List<Map<String, String>>> CHILD_BATCH = null;
	...
	public static List<Integer> getBatches(int pendingCount, int noOfBatch, int recordsPerbatch)
		...
		if (pendingCount > (noOfBatch * recordsPerbatch)) {
			child_records_count = pendingCount - (noOfBatch * recordsPerbatch); // 2 records
	...
	public String testBatch() throws SQLException, InterruptedException {
		...
		PARENT_ID = objUtil.addBatchLogCust("PB");
		ResultSet rs = null;
		Integer db = 0;
		List<Map<String, String>> pendingData = null;
		ResultSet resultSet = null;

		try (Connection connection = DBUtil.dataSourceClient.getConnection();) {
			Statement stmt3 = connection.createStatement();
			// U_ID List
			Statement stmt2 = connection.createStatement();
			resultSet = stmt2.executeQuery(
					"SELECT U_ID FROM  XX_ETQ_CUSTOMER_PROFILE_DETAIL WHERE (ETQ_UPLOAD_STATUS=1 OR ETQ_UPLOAD_STATUS IS NULL OR ETQ_UPLOAD_STATUS=0) AND (ETQ_BATCH_ID IS NULL OR ETQ_BATCH_ID=0)");
			...
			if (pendingData.isEmpty()) {
				...
				objUtil.updateBatchLogCust(PARENT_ID, 0, endDate_parent, "COMPLETED",
				...
				rs = stmt.executeQuery(
						"SELECT COUNT(*) as DATA_COUNT FROM XX_ETQ_CUSTOMER_PROFILE_DETAIL  WHERE (ETQ_UPLOAD_STATUS=1 OR ETQ_UPLOAD_STATUS IS NULL OR ETQ_UPLOAD_STATUS=0) AND (ETQ_BATCH_ID IS NULL OR ETQ_BATCH_ID=0)");
				...
				for (int i = 0; i < actualChildBatchCount.get(0); i++) {
					int B_Id = 0;
					B_Id = objUtil.addBatchLogCust("ABL_CR");
					B_Id_List.add(B_Id);
					...
					for (Integer uId : indBatchList) {
						stmt3.executeUpdate("UPDATE XX_ETQ_CUSTOMER_PROFILE_DETAIL SET ETQ_BATCH_ID = " + B_Id
								+ " WHERE U_ID = " + uId);
					}
					indBatchList.clear();
				...
				if (actualChildBatchCount.get(1) != 0) {
					for (int i = 0; i < actualChildBatchCount.get(1); i++) {
						int B_Id = 0;
						B_Id = objUtil.addBatchLogCust("CBL_CR");
						B_Id_List.add(B_Id);
						for (int j = 0; j < scheduler.getNO_OF_RECORD(); j++) {
							if (Uid_list.size() >= 1) {
...
//			DbUtils.closeQuietly(stmt);
			...
			utility.exceptionLog("CustomerBatchService", "testBatch", "", LOGGER, null, "", e.getMessage());
		}
	...
	private void executorThreadprocessForEMAData(List<Integer> B_Id_List, ExecutorService executor)
		...
		try {
			...
			for (Iterator<Integer> B_Id_Iterator = B_Id_List.iterator(); B_Id_Iterator.hasNext();) {
				...
				executor.execute(new multithread(B_Id));
			}

		} catch (Exception e) {
			utility.exceptionLog("executorThreadprocessForEMAData", "testBatch", "", LOGGER, null, "", e.getMessage());
		}
	}
}
